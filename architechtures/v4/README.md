...
scripts/train_qwen_sft.py
                              Full-model Qwen/Qwen-style SFT trainer
scripts/eval_qwen_sft.py
                              Masked-loss evaluation and sample generation
scripts/prepare_qwen_posttrain_data.py
                              Clean/tokenize/split post-training chat data
...
Qwen SFT data-contract tests:

```bash
uv run -- pytest test/test_train_qwen_sft.py
```
...
The repo also includes a lean full-model SFT path for Qwen-style chat models.
It masks prompt tokens with `-100`, trains the entire base model, and saves
both intermediate checkpoints and a final `save_pretrained()` directory.

Minimal smoke run:

```bash
uv run -- python -m scripts.train_qwen_sft \
  --train-file runs/qwen_sft_smoke_chat.jsonl \
  --output-dir runs/qwen35_08b_full_sft_smoke \
...
```bash
uv run -- python -m scripts.eval_qwen_sft \
  --model runs/qwen35_08b_full_sft_smoke \
  --train-file runs/qwen_sft_smoke_chat.jsonl \
  --show-samples 2
```
...
```bash
uv run -- python -m scripts.prepare_qwen_posttrain_data \
  --input-file /workspace/jepa-code/architechtures/v3/data/nemotron_v2_chat_64.jsonl \
  --output-dir runs/posttrain_nemotron_chat64_clean \
  --model-name Qwen/Qwen3.5-0.8B \
  --max-length 2048 \
  --category chat \
  --reasoning-filter off \
  --train-count 48 \
  --val-count 16
```

The prep script applies the same `apply_chat_template` and prompt-prefix masking
contract used by training, removes duplicate message sequences, strips empty
`<think></think>` scaffolding from assistant answers, drops overlength examples,
and writes deterministic train/val JSONL splits plus a manifest with token stats.

Notes:

- `--gradient-checkpointing` is enabled by default for the full 0.8B model.
- The default checkpoint state is intentionally lightweight; optimizer state is
  only saved if `--save-optimizer-state` is passed.
- `--train-file` expects JSONL rows with either a `messages` list ending in an
  assistant turn, or prompt/solution-style fields that the script can map into
  a two-turn chat example.
